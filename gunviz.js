...
function loadData() {
	...
	$.getJSON('data/curves.json', onLoaded);
}
...
function initData() {
	//get the data sorted by order and ready to display
...
//When page loads, set up all the tweens inside a for loop through the data array, with delay incremented as a function of the loop that establishes the tweens (i.e. delay constantly increasing, but by decreasingly small amounts)
//each curve is driven by two bezier-shape tweens: the first shapes the "life" curve, the second shapes the "afterlife" curve
//Tweens will increment properties of the curves bit by bit, meanwhile easel checks in every so often and renders those properties to the canvas.
...
function tick() {
	...
	//on easel tick update, for each line currently being drawn, move to that line's endpoint (stored in an array previously) and draw a line to the new (x, y) value calculated by TweenMax.
	...
	for (var i = 0; i < active_len; i++) {
		...
		//fuzzy mode:
/*		y = Math.random() < 0.5 ? activecurves[i].y - Math.random()*50 : activecurves[i].y + Math.random()*50;
*/
		//don't plot extra-long lines
/*		if (x - mrx > 75) {
			continue;
		}*/
...
function removeCurve(i, curve) {
	...
	yearsCount += Math.floor(curve.altage) - Math.floor(curve.age);
	...
	if (i === 4) {
		if (typeof introTtip[0] != "undefined") {
			//introTtip[0].el().remove();
		}
		if (typeof introTtip[1] != "undefined") {
			//introTtip[1].el().remove();
		}
	}
...
function updateLabels() {
	...
	//document.getElementById("numTop").innerHTML = Math.round(pctTop).toString();
	document.getElementById("pctBot").innerHTML = formatComma(Math.round(pctBot))+" | "+Math.round(pctBot / peopleCount * 100).toString()+"%";
	//document.getElementById("numBot").innerHTML = Math.round(pctBot).toString();

	document.getElementById("monthLabel").innerHTML = monthLabel == 0 ? "" : monthDict[monthLabel];
	
	if (peopleCount == curves.length) {
		document.getElementById("monthLabel").innerHTML = "";
	}
}
...
function getInfo(x, y) {
	...
	var race = raceDict[data.vrace];
	if (data.age > 18) {
		var sex = sexDict[data.vsex];
	} else if (data.age > 1) {
		switch (data.vsex) {
			case "M": 
				var sex = "boy";
	...
	var month = " in "+bEm+monthDict[data.month]+eEm;
	if (data.state === "UK") {
		var state = "";
	} else {
		var state = " in "+bEm+stateDict[data.state]+eEm;
	}

	var age = Math.floor(data.age);
	var posPronoun = posPronounDict[data.vsex];
	var relationship = relationDispDict[data.osex][data.offrel];
	var pronoun = pronounDict[data.vsex];
	...
	if (groupNum <= 0) {
		var pronounPhrase = posPronoun+" "+relationship;
	} else if (groupNum == 1) {
	...
	var weapon = data.weapon.substr(0) === "u" ? "n "+data.weapon : " "+data.weapon;
	var altage = Math.floor(data.altage);

	var tiptext1 = ["This ",bEm,race," ",sex,eEm," was shot",month,state," at the age of ",bEm,age,eEm," by ",bEm,pronounPhrase,eEm,circumstances,".",othersListing];
	var tiptext2 = [bIt,"Had ",pronoun," not been killed with a",bEm,weapon,eEm,", ",pronoun," might have lived to be ",bEm,altage,eEm," and died of ",altCause,".",eIt];
...
function curveFilter() {
	...
	this.handgun = function(item) {if (item.weapon == "handgun") {return true;} else {return false;}}
	this.shotgun = function(item) {if (item.weapon == "shotgun") {return true;} else {return false;}}
	this.rifle = function(item) {if (item.weapon == "rifle") {return true;} else {return false;}}
	this.other = function(item) {if (item.weapon == "unknown gun" || item.weapon == "firearm") {return true;} else {return false;}}
	...
	this.male = function(item) {if (item.vsex == "M") {return true;} else {return false;}}
	this.female = function(item) {if (item.vsex == "F") {return true;} else {return false;}}
	this.black = function(item) {if (item.vrace == "B") {return true;} else {return false;}}
	this.white = function(item) {if (item.vrace == "W") {return true;} else {return false;}}
	this.amindian = function(item) {if (item.vrace == "I") {return true;} else {return false;}}
	this.asian = function(item) {if (item.vrace == "A") {return true;} else {return false;}}
	this.midwest = function(item) {if (item.region == "MW") {return true;} else {return false;}}
	this.northwest = function(item) {if (item.region == "NW") {return true;} else {return false;}}
	this.northeast = function(item) {if (item.region == "NE") {return true;} else {return false;}}
	this.southwest = function(item) {if (item.region == "SW") {return true;} else {return false;}}
	this.southeast = function(item) {if (item.region == "SE") {return true;} else {return false;}}
	this.single = function(item) {if (item.multiple == 0) {return true;} else {return false;}}
	this.multiple = function(item) {if (item.multiple == 1) {return true;} else {return false;}}
...
function getSubset(passCurves, filterCriterion) {
	...
	for (var i = 0, clen = passCurves.length; i < clen; i++) {
		if (filterFunction(passCurves[i])) {
			...
			peopleCount++;
			yearsCount += Math.floor(passCurves[i].altage) - Math.floor(passCurves[i].age);
	}
...
function makeDist(subset) {
	...
	for (var a = 0; a < 2; a++) {
		for (var b = 0; b < 2; b++) {
			for (var c = 0; c <= 100; c++) {
				histValues[a][b][c] = {filter: subset[2].filter, total: 0, value: 0};
			}
		}
	}
...
function getDistTipText(x) {
	...
	var topBoilerplate = [bEm,numTop," ",insert1,eEm," were killed",insert2,"<hr class='hoverRule'>",bIt,onlyTop,bEm,altNumTop,eEm," of ",formatComma(pctTop)," ",insert1," could have died at age ",x," of alternative causes.",eIt];
	var midBoilerplate = ["age ",x];
	var botBoilerplate = [bEm,numBot," ",insert3,eEm," were killed",insert4,"<hr class='hoverRule'>",bIt,onlyBot,bEm,altNumBot,eEm," of ",formatComma(pctBot)," ",insert3," could have died at age ",x," of alternative causes.",eIt];
